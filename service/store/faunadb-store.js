...
module.exports = class Store {
  ...
  async createApiKey(tenant, name, tag, primary, secondary) {
    ...
    try {
      ...
      const exists = await client.query(
        q.Exists(q.Match(q.Index("api-key-by-tag"), q.Casefold(tenant), tag))
      )
  ...
  async createTenant(name) {
    ...
    // Don't create a key if one already exists.
    const exists = await client.query(
      q.Exists(q.Match(q.Index("tenant-by-tag"), tag))
    )
    if (exists === true) return null

    const result = await client.query(
      q.Create(q.Collection("tenants"), {
        data: {
          tag,
          name,
        },
  ...
  async deleteApiKey(tenant, tag) {
    ...
    try {
      ...
      const existing = await client.query(
        q.Get(q.Match(q.Index("api-key-by-tag"), q.Casefold(tenant), tag))
      )
          ...
          q.Let(
            {
              ...
              entityDoc: q.Get(
                q.Select(["data", "entity"], q.Var("revisionDoc"))
              ),
            ...
            q.If(
              q.Equals(
                q.Var("revisionRef"),
                q.Select(["data", "latest"], q.Var("entityDoc"))
              ),
              q.If(
                q.Equals(
                  "draft",
                  q.Select(["data", "status"], q.Var("revisionDoc"))
                ),
                q.If(
                  ...
                  q.Let(
                    {
                      ...
                      updated: q.Update(q.Select(["ref"], q.Var("entityDoc")), {
                        data: {latest: q.Var("headRef")},
                      }),
                    },
                    {
                      status: "success",
                  ...
                  q.Let(
                    {
                      deletedEntity: q.Delete(
                        q.Select("ref", q.Var("entityDoc"))
                      ),
  ...
  async getApiKeys(tenant) {
    ...
    const helper = client.paginate(
      q.Match(q.Index("api-keys-summary"), q.Casefold(tenant))
    )
  ...
  async getEntities(tenant) {
    ...
    return result.data.map((e) => ({
      ...
      latest: {
        ...e.latest,
        last_modified_on: moment(e.latest.last_modified_on / 1000).format(
          "YYYY-MM-DD"
        ),
      },
      head: {
        ...e.head,
        last_modified_on: moment(e.head.last_modified_on / 1000).format(
          "YYYY-MM-DD"
        ),
      },
    }))
  ...
  async getEntity(id) {
    ...
    return {
      ...
      revision: {
        ...
        last_modified_on: moment(result.data.revision.ts / 1000).format(
          "YYYY-MM-DD"
        ),
      },
    }
      ...
      q.Let(
        ...
        q.If(
          ...
          q.Let(
            {
              entityDoc: q.Get(q.Var("entityRef")),
              revisionRef: q.Match(q.Index("entity-by-revision"), [
                q.Select("ref", q.Var("entityDoc")),
                revision,
              ]),
            },
          ...
          q.Let(
            {
              entityDoc: q.Get(q.Var("entityRef")),
              revisionDoc: q.Get(
                q.Select(["data", "head"], q.Var("entityDoc"))
              ),
            },
            {
              status: "success",
              ...
              q.Let(
                ...
                q.If(
                  ...
                  q.If(
                    ...
                    q.Let(
                      {
                        newRevision: q.Add(
                          1,
                          q.Select(["data", "revision"], q.Var("revisionDoc"))
                        ),
                        ...
                        updateResult: q.Update(
                          ...
                          {
                            data: {
                              latest: q.Select("ref", q.Var("createdResult")),
                            },
                          }
                        ),
                      },
                    ...
                    q.Let(
                      {
                        updateResult: q.Update(q.Var("revisionRef"), {
                          data: {
                            edited_by: userId,
                            definition,
                          },
  ...
  async updateUserLogin(ref, issuer, timestamp) {
    const client = this._getClient()
    await client.query(
      q.Update(ref, {data: {id: issuer, lastLoginAt: timestamp}})
    )
  }
